...
<div>
    ...
    <div class="inner">
      <div class="head">
        <div class="author">
          <span>John Barrientos</span>
          <span @click="closeFeedback()" class="closer"><i class="las la-times"></i></span>
        </div>
        <div @click="toggleMark(selected)" class="status" v-bind:class="{'done' : selected.mark_as_finished}">
      ...
      <div class="response-list">
          <div v-for="response in selected.chapter_feedback_responses" :key="response.uuid" class="response-single">
            <strong>John Barrientos</strong>
            <p style="margin:0px;">{{ response.message }}</p>
          </div>
      </div>
      <div class="foot">
  ...
  methods: {
    ...
    saveFeedback: function () {
      ...
      let feedback = {
        ...
        book_id: scope.properties.book.uuid,
        feedback_id: null,
        mark: 'unmarked',
        published: 1,
        type: 'post',
      ...
      scope.axios
        .post('http://localhost:3000/book-feedbacks', feedback)
        .then(response => {
          if (response.data) {
            scope.message = ''
